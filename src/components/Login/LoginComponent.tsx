...
import { variant1 } from "../RegisterComponents/VariantStep";
...
export default function LoginComponent() {
  ...
  const createUser = (data: Inputs) => {
    validateLogin
    const userData = { ...data, name: data.name.trim() }; 
    localStorage.setItem("userData", JSON.stringify(userData));
    navigate("/");
  };

  const validateLogin = (data: Inputs) => {
    ...
    if (
      ...
      toast("email e senha devem ser iguais ao do seu cadastro")
    }
  };
  ...
  return (
    ...
    >
      <div className="flex justify-center items-center h-screen">
        <form
          className="w-full md:w-3/4 lg:w-2/4 xl:w-1/3 p-8 bg-azul-celeste bg-opacity-50 rounded-lg shadow-lg"
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <button className="button bg-Marrom w-full transform hover:scale-105 transition duration-300 ease-in-out">
              Cadastro
            </button>
            <Link to="/login" className="underline text-white">
              Se não possui conta crie uma
            </Link>
          </div>
        </form>
        <ToastContainer
